...
async function markdownToDocxParagraphs(markdown: string): Promise<Paragraph[]> {
  ...
  for (const token of tokens) {
    if (token.type === 'paragraph') {
      const html = await marked.parseInline(token.text);
      const runs: TextRun[] = [];
      let bold = false;
      let italics = false;
      html.split(/(<\/strong>|<strong>|<\/em>|<em>|<br\s*\/?>)/).forEach((part) => {
        if (part === '<strong>') {
          bold = true;
        } else if (part === '</strong>') {
          bold = false;
          ...
          italics = false;
        } else if (part && !part.startsWith('<br')) {
          const clean = decodeHtmlEntities(part.replace(/<[^>]+>/g, ''));
          runs.push(new TextRun({ text: clean, bold, italics }));
        }
      });
...
export function DashboardMainContent({
  ...
  product,
  objective,
}: Omit<DashboardMainContentProps, 'onSaveReport'>) {
  ...
  async function downloadReportAsDocx() {
    if (!selectedSegment || !responses[selectedSegment]) {
      return;
    }
    const report = {
      title: `SWOT Analysis - ${selectedSegment} - ${product} - ${objective}`,
    ...
    const children: Paragraph[] = [
      new Paragraph({
        children: [
          new TextRun({ text: report.title, bold: true, size: 32 }),
        ],
      }),
    ];
    for (const insight of report.insights) {
      const categoryHeading = new Paragraph({
        children: [
          new TextRun({ text: `${insight.category}:`, bold: true, size: 28 }),
        ],
        spacing: { after: 120 },
      });
      const paragraphs = await markdownToDocxParagraphs(insight.content);
      children.push(categoryHeading, ...paragraphs);
    }
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {segmentResponses && !pathname.includes('/reports') && (
            <Button onClick={downloadReportAsDocx} variant="outline">
              💾 Save Report
            </Button>
          )}
        </div>
